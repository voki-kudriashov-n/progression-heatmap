...
Current pre-group aggregation filters are attempt group, platform, traffic type, payer type, and `super_ball`. The dashboard applies staged filter changes to the graph only when the user clicks `Применить`. Level cohort range and date range are display filters applied after grouped statistics are cached. Percentage heatmap cells include numerator/sample counts in hover details. If the selected minimum observation threshold is greater than `0`, low-sample cells are hidden before rendering and do not affect the heatmap color gradient.
